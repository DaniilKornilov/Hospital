...
namespace Hospital
{
    public partial class Diagnosis : Form
    {
        ...
        private readonly string selectSQL = "Select * from Diagnosis";
        private readonly Form form;

        public Diagnosis(Form form)
        {
            InitializeComponent();
            form.Hide();
            this.form = form;
        ...
        private void dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView.CurrentRow != null)
            {
                try
                {
                    ...
                    using OracleCommand oracleCommand = new OracleCommand("DiagnosisAddOrEdit", connection);
        ...
        private void dataGridView_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Are you sure to delete this diagnosis?", "Diagnosis", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    try
                    {
                        using OracleConnection connection = new OracleConnection(connectionString);
                        connection.Open();
                        using OracleCommand oracleCommand = new OracleCommand("DiagnosisDeleteByID", connection);
                        oracleCommand.CommandType = CommandType.StoredProcedure;
